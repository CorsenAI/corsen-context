...
const repositoryUrl =
  process.env.CORSEN_CONTEXT_REPOSITORY_URL ||
  'https://github.com/CorsenAI/corsen-context-static-html';
...
export function renderDocument(page, options = {}) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(page.title)}${page.path !== '/' ? ' — Corsen Context' : ''}</title><meta name="description" content="${esc(page.description)}"><style>${styles}</style><link rel="stylesheet" href="/corsen/cc-nav.css"><link rel="stylesheet" href="/corsen/cc-observatory.css">${agentScripts}</head><body><div data-cc-nav data-stack="Static HTML" data-uid="html" data-home="/" data-accent="#0f766e" data-repository="${esc(repositoryUrl)}"></div><main id="top">${body}<section id="live" style="padding:32px 24px 8px"><h2>Live contract observatory</h2><div data-cc-observatory data-stack="Static HTML" data-endpoint="/v1/mcp" data-query="Static HTML" data-accent="#0f766e"></div></section></main><footer data-cc-foot data-stack="Static HTML" data-accent="#0f766e" data-repository="${esc(repositoryUrl)}"></footer><script>${interactions}</script><script src="/corsen/cc-observatory.js" defer></script><script src="/corsen/cc-nav.js" defer></script></body></html>`;
}
