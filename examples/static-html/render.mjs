...
export function renderDocument(page, options = {}) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(page.title)} — Aurora Kits</title><meta name="description" content="${esc(page.description)}"><style>${styles}</style>${agentScripts}</head><body><header class="site-header"><a class="brand" href="/" aria-label="Aurora Kits home"><span aria-hidden="true">AK</span>Aurora Kits</a><nav aria-label="Demo pages"><ul>${navigation.map(([href, label]) => `<li><a href="${href}"${page.path === href ? ' aria-current="page"' : ''}>${label}</a></li>`).join('')}</ul></nav></header><main>${body}</main><footer><p>${surfaces}</p><p>Static HTML reference integration powered by Corsen Context.</p></footer><script>${interactions}</script></body></html>`;
}
